...
struct AdaptiveThreshold {
    ...
    struct ThresholdObservation: Codable, Sendable {
        let success: Bool
        let iterations: Int
        let context: String
        let timestamp: Int64
    ...
    private mutating func calibrateThreshold() {
        ...
        let successRate = Double(recent.filter { $0.success }.count) / Double(recent.count)
        let avgIterations = Double(recent.reduce(0) { $0 + $1.iterations }) / Double(recent.count)

        let adjustment: Double
        if successRate > 0.9 && avgIterations < 1.2 { adjustment = 0.02 }
        else if successRate < 0.5 { adjustment = -0.03 }
        else { adjustment = 0.0 }
    ...
    func getStats() -> (threshold: Double, observations: Int, recentSuccessRate: Double) {
        ...
        if !recent.isEmpty {
            successRate = Double(recent.filter { $0.success }.count) / Double(recent.count)
        } else { successRate = 0.8 }
...
struct FailurePatternLibrary {
    ...
    mutating func pruneStale() {
        let staleKeys = patterns.filter { $0.value.isStale }.map { $0.key }
        for key in staleKeys { patterns.removeValue(forKey: key) }
    }
    ...
    private mutating func regenerateRules() {
        ...
        for (modelId, contextCounts) in modelContextCounts {
            for (context, count) in contextCounts where count >= FailurePatternLibrary.ruleGenerationThreshold {
                let priority: PreventionPriority
...
struct PreventionRule: Codable, Sendable {
...
enum PreventionPriority: String, Codable, Sendable {
