...
enum IntentUrgency: String, Codable, Sendable {
...
struct ExtractedIntent: Sendable, Codable {
...
struct IntentExtractor: Sendable {
    ...
    func extract(from text: String) -> ExtractedIntent {
        ...
        if let highest = actionScores.max(by: { $0.value < $1.value }) {
            dominantAction = highest.key
        } else {
            // Default: treat as question if no clear action
            dominantAction = lowerText.hasSuffix("?") || text.hasSuffix("?")
                ? .askQuestion : .other
        }
    ...
    private func extractTarget(from text: String, action: IntentAction) -> String? {
        let lowerText = text.lowercased()
