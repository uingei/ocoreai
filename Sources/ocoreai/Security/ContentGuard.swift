...
public struct GuardResult: Sendable, Codable {
    ...
    public var isBlocked: Bool { !passed }
...
public enum SafetyCategory: String, Codable, Sendable, CaseIterable {
    ...
    public var severity: Double {
        switch self {
        case .underageSexual, .sexualViolence: return 1.0    // Always block
        case .selfHarm, .jailbreak, .systemPromptOverride: return 0.95
        case .sexuallyExplicit, .graphicViolence, .hateSpeech: return 0.9
        case .toolAbuse, .illegalActivity, .malwareGeneration: return 0.85
        case .piiRequest: return 0.7
        }
    }

    public var description: String {
        switch self {
        case .sexuallyExplicit: return "Sexually explicit content detected"
        case .sexualViolence: return "Sexual violence content detected"
        case .underageSexual: return "Underage sexual content detected"
        case .graphicViolence: return "Graphic violence detected"
        case .selfHarm: return "Self-harm content detected"
        case .hateSpeech: return "Hate speech detected"
        case .jailbreak: return "Jailbreak attempt detected"
        case .systemPromptOverride: return "System prompt manipulation detected"
        case .toolAbuse: return "Dangerous tool use detected"
        case .illegalActivity: return "Illegal activity facilitation detected"
        case .malwareGeneration: return "Malware/ exploit generation detected"
        case .piiRequest: return "PII extraction attempt detected"
        }
    }
}
...
public enum DetectionMode: String, Codable, Sendable {
    ...
    public static func defaultFor(_ category: SafetyCategory) -> DetectionMode {
        ...
        case .underageSexual, .sexualViolence, .selfHarm:
            return .strict  // Never disable these
        case .jailbreak, .systemPromptOverride, .toolAbuse:
            return .strict  // System integrity
...
struct RuntimeSafetyConfig: Sendable {
    let enabled: Bool
    ...
    init(from config: SafetyConfig) {
        ...
        self.minMatchesRequired = config.minMatchesRequired
        self.logRedaction = config.logRedaction
        ...
        self.additionalKeywords = keywords
    }
...
public actor ContentGuard {
    ...
    nonisolated(unsafe) private var checksRun: Int64 = 0
    nonisolated(unsafe) private var blocksTriggered: Int64 = 0
    ...
    init(runtimeConfig: RuntimeSafetyConfig) {
        ...
        self.keywordSets = Self.mergeKeywords(
            builtins: builtins,
            additional: runtimeConfig.additionalKeywords
        )

        // Build regex patterns
        self.regexPatterns = Self.compileRegexPatterns()
    }
    ...
    public func checkInput(_ text: String) -> GuardResult {
        let rtConfig = self.runtimeConfig
    ...
    private func scanRegex(_ lowerText: String, text originalText: String) -> [SafetyCategory: Int] {
        var hits: [SafetyCategory: Int] = [:]
