...
struct SystemHealth: Codable {
	...
	var isHealthy: Bool {
		overallScore >= SelfAdaptationConfig.healthyThreshold
	}

	var needsAttention: Bool {
		overallScore < SelfAdaptationConfig.warningThreshold
	}
...
struct SelfAdaptationConfig: Codable {
...
actor AgentSelfAdaptationActor {
	...
	private struct ModelHealthProfile: Codable {
	...
	init(enabled: Bool, healthScore: Double, log: Logger = Logger(label: "ocoreai.adaptation")) {
		isEnabled = enabled
		logger = log
		health = SystemHealth.initial()
		health.overallScore = healthScore
		adaptiveThreshold = AdaptiveThreshold()
		failureLibrary = FailurePatternLibrary()
	}
	...
	func preInferenceCheck(modelId: String) -> InferenceRecommendation {
		...
		let recommendation: InferenceRecommendation = if health.overallScore >= SelfAdaptationConfig.healthyThreshold, !isModelStressed {
			.proceed
	...
	func periodicEvaluation() {
		guard isEnabled else { return }
		let stressCount = healthHistory.count(where: { $0 < SelfAdaptationConfig.warningThreshold })
...
enum InferenceRecommendation: String {
	case proceed
