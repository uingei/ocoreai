...
/// 运行中 endpoint 的状态封装。
private struct EndpointHandle: Sendable {
    /// Endpoint 配置
...
actor MCPBridge {
    ...
    private func handleBridgeCommand(_ line: String) async -> String? {
        ...
        case "$bridge/cache/status":
            return jsonResult(await callCache.status(), id: reqID)
        default:
            return nil
        }
    }
    ...
    private func callLocalTool(_ toolName: String, arguments: String) async throws -> String {
        ...
        guard let reqJSON = try? JSONSerialization.data(withJSONObject: jsonRpcReq, options: .sortedKeys),
              let reqStr = String(data: reqJSON, encoding: .utf8) else {
        ...
        guard let response = await server.dispatch(reqStr),
              ...
              let contents = result["content"] as? [[String: Any]] else {
        ...
        return try await withThrowingTaskGroup(of: String.self) { group in
            for (clientName, client) in resolvedClients {
                ...
                group.addTask { @Sendable [weak self] in
                    ...
                    let result = try await s.forwardToolCall(to: client, tool: toolName, args: argsJsonStr)
                    return result
                }
            }
    ...
    private func routeFirstResponse(
        ...
        let result = try await routeSerial(toolName, arguments: arguments, names: names)
        return result
    }
    ...
    private func bridgeStatus() -> [String: Any] {
        var connected = 0
        for handle in endpointHandles.values where handle.status == .connected { connected += 1 }
...
public enum MCPBridgeError: Error, Sendable {
    ...
    var localizedDescription: String {
        ...
        case .routingFailed(let tool, let reason):
            return "Tool '\(tool)' routing failed: \(reason)"
        case .noServerAvailable(let tool):
            return "No MCP server available for tool '\(tool)'"
        case .allExternalServersFailed(let tool):
            return "All external servers failed for tool '\(tool)'"
        case .externalProcessNotManaged:
            return "External process management not yet integrated"
        }
    }
}
