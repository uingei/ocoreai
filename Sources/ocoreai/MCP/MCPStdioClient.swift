...
/// 外部 MCP server 的连接状态
enum MCPClientConnectionStatus: String, Sendable, Codable {
...
actor MCPStdioClient {
    ...
    private func waitForResponse() async throws -> String {
        let deadline = ContinuousClock.now + .milliseconds(15_000)
...
/// 外部 MCP 客户端错误
enum MCPClientError: Error, Sendable, LocalizedError {
    ...
    var errorDescription: String? {
        ...
        case .notConnected(let name):
            return "MCP client for '\(name)' is not connected"
        case .timeout(let name):
            return "Request timed out for MCP client '\(name)'"
