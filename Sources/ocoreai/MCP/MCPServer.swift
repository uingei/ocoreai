...
private struct JVal: Codable {
    ...
    init(from decoder: Decoder) throws {
        ...
        else if let a = try? c.decode([JVal].self) { value = a.map { $0.value } }
...
private enum RpcErr: Int {
    ...
    var msg: String {
        ...
        case .noMethod: return "Method not found"
        case .badParams: return "Invalid params"
        case .internalServerError: return "Internal error"
        }
    }
}
...
actor MCPServer {
    ...
    private func doInit(_ p: [String: Any]?) -> JVal {
        ready = true
        return JVal([
    ...
    private func res(_ v: JVal, id: String?) -> String {
        return enc(["result": v.value], id: id)
    }
    
    private func err(_ e: RpcErr, id: String?) -> String {
        return enc(["error": ["code": e.code, "message": e.msg]], id: id)
    }
