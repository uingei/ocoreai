...
private struct ComparableRequest: Comparable, Equatable {
    ...
    var priorityRaw: Int { request.priority.rawValue }
    var createdAtTime: TimeInterval { request.createdAt.timeIntervalSince1970 }
...
actor SchedulerActor {
    ...
    func submitAll(_ requests: [SchedulingRequest]) async throws -> [String] {
        ...
        for request in requests {
            try ids.append(await submit(request))
        }
        return ids
    }
    ...
    func snapshot() async -> SchedulerSnapshot {
        let memUsage: Double
        if let tracker = memoryTracker {
            memUsage = await tracker.usageFraction()
        } else {
            memUsage = 0
        }
        let level: String
        if let oomg = oomGuard {
            level = await oomg.currentQuantization().rawValue
        } else {
            level = "unknown"
        }
    ...
    var activeCount: Int { activeRequests.count }
...
public enum SchedulerError: Error, LocalizedError, Sendable, Equatable {
    ...
    public var errorDescription: String? {
        switch self {
        case .queueFull: return "Scheduler queue is full"
        case .oomRefused: return "Request refused due to OOM protection"
        case .notFound(let id): return "Request not found: \(id)"
        case .timeout(let id): return "Request timed out: \(id)"
        }
    }
}
