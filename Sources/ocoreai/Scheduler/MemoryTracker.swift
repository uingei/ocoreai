...
actor MemoryTracker {
    ...
    public func setOOMCallback(_ callback: @escaping @Sendable (MemoryLevel) async -> Void) {
        self.oomCallback = callback
    }
    ...
    public func allocation(_ bytes: UInt64) {
        usedBytes += bytes
        checkLevel()
    }
    ...
    public func deallocation(_ bytes: UInt64) {
    ...
    public func reportUsage(_ bytes: UInt64) {
        usedBytes = bytes
        checkLevel()
    }
    
    /// Snapshot of current memory state.
    public func snapshot() -> MemoryLevel {
        currentLevel
    }
