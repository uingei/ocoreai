        ...
        VStack(alignment: .leading, spacing: 8) {
            HStack {
                ...
                Toggle(
                    "",
                    isOn: Binding(
                        get: { mmState.cameraEnabled },
                        set: { MultimodalState.shared.cameraEnabled = $0 })
                )
                .toggleStyle(.switch)
        ...
        VStack(alignment: .leading, spacing: 8) {
            HStack {
                Image(systemName: "desktopcomputer")
                    .foregroundStyle(
                        screenshotService.isCapturing ? theme.greenDot : theme.textSecondary
                    )
                    .accessibilityHidden(true)
        ...
        VStack(alignment: .leading, spacing: 8) {
            ...
            if mmState.microphoneEnabled {
                Button(action: {
                    ...
                    HStack {
                        Image(
                            systemName: audioIO.isRecording ? "stop.circle.fill" : "record.circle"
                        )
                        .font(.title3)
                        Text(
                            audioIO.isRecording
                                ? StringKey.stopRecordingLabel.l : StringKey.startRecordingLabel.l)
                    }
                    .frame(maxWidth: .infinity)
                }
                .buttonStyle(.borderedProminent)
                ...
                .accessibilityHint(
                    audioIO.isRecording
                        ? StringKey.stopRecordingHint.l : StringKey.startRecordingHint.l)
            }
            ...
            if !audioIO.partialText.isEmpty {
                Text(audioIO.partialText)
                    ...
                    .lineLimit(3)
                    .accessibilityLabel(
                        "\(StringKey.multimodalSTTPartialLabel.l): \(audioIO.partialText)"
                    )
        ...
        VStack(alignment: .leading, spacing: 8) {
            HStack {
                ...
                Toggle(
                    "",
                    isOn: Binding(
                        get: { mmState.speakerEnabled },
                        set: { MultimodalState.shared.speakerEnabled = $0 })
                )
                .toggleStyle(.switch)
...
extension MultimodalControls {
    private struct StatusDot: View {
        ...
        var body: some View {
            ...
            .accessibilityLabel(
                "\(label): \(isActive ? StringKey.statusActive.l : StringKey.statusInactive.l)"
            )
            .accessibilityAddTraits(.isStaticText)
        }
    }
