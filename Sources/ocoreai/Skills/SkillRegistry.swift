...
actor SkillRegistry {
	...
	func bootstrap(skillsDir: URL?, systemPromptBuilder: SystemPromptBuilder? = nil) async throws {
		...
		for builtin in BuiltInSkill.allCases {
			...
			register(skill, skipCheck: true)
		}

		// Step 2: Scan disk skills if directory provided
		if let skillsDir {
			_ = loadDiskSkills(in: skillsDir, incremental: false)
		}
	...
	func reloadSkills(skillsDir: URL) async {
		_ = loadDiskSkills(in: skillsDir, incremental: true)
		resolveIfNeeded()
		if let callback = hotReloadCallback {
			await callback()
		}
	}
	...
	private func resolveIfNeeded() {
		guard resolvedDirty else { return }

		do {
			cachedResolvedOrder = try resolveOrder()
			resolvedDirty = false
		} catch {
			logger.error("Dependency resolution warning: \(error.localizedDescription)")
			cachedResolvedOrder = Array(skills.keys)
			resolvedDirty = false
		}
	}
	...
	private func resolveOrder() throws -> [String] {
		let allNames = Array(skills.keys)
		...
		for name in allNames {
			guard let skill = skills[name] else { continue }
	...
	private func register(_ skill: Skill, skipCheck _: Bool = false) {
		guard let existing = skills[skill.name] else {
			skills[skill.name] = skill
			indexByCategory[skill.category, default: []].append(skill.name)
			for tag in skill.metadata.tags {
				indexByTag[tag, default: []].append(skill.name)
			}
			markDirty()
			return
		}
		...
		if var cats = indexByCategory[existing.category] {
			cats.removeAll { $0 == skill.name }
			indexByCategory[existing.category] = cats
		}
		for tag in existing.metadata.tags {
			if var tags = indexByTag[tag] {
	...
	func remove(_ name: String) -> Skill? {
		guard let skill = skills.removeValue(forKey: name) else { return nil }

		if var cats = indexByCategory[skill.category] {
			cats.removeAll { $0 == name }
			indexByCategory[skill.category] = cats
		}
		for tag in skill.metadata.tags {
			if var tags = indexByTag[tag] {
	...
	private func loadDiskSkills(in dir: URL, incremental: Bool) -> Int {
		...
		for skill in discovered {
			...
			if let modDate = try? url.resourceValues(
				...
				fileMtimes[url] = modDate
			}
		}

		if newCount > 0 {
			logger.info("Loaded \(newCount) disk skill(s) from \(dir.path)")
		}
