...
public final class OcoreaiEngine {
    ...
    public func start() async {
        ...
        // Priority: env var > UserDefaults (persistent form). env-first mirrors upstream (no keychain tokens).
        let msToken: String? =
        ...
        // Verify 段 P0（审计 trace 持久化）：绑定已打开的
        ...
        _toolRegistry = ToolRegistry(
            ...
            hooks: [
                .matching(
                    "write_file,edit_file,exec_command,execute_code,exec_shell,write_stdin,exec_poll"
                ) { _ in
