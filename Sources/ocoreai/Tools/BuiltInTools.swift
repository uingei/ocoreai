...
func bootstrapBuiltInTools(
    ...
    try? await registry.register(ToolEntry(
        ...
        handler: { args in
            ...
            let result: String
            switch topic {
            case "status": result = "ocoreai runtime v0.7.0 — healthy"
            case "version": result = "0.7.0"
            case "uptime": result = "uptime: \(ProcessInfo.processInfo.systemUptime)"
            default: result = "topic '\(topic)' not recognized"
            }
            return result
        }
    ))
        ...
        try? await registry.register(ToolEntry(
            ...
            handler: { [sr] args in
                ...
                if let cat = category, !cat.isEmpty {
                    names = await sr.lookupCategory(cat).map { $0.name }
                } else {
                    names = await sr.list()
                }
                if names.isEmpty {
    ...
    try? await registry.register(ToolEntry(
        ...
        handler: { args in
            let message = parseArgKey(args, key: "message") ?? ""
            return message
        },
        isDestructive: false
    ))
