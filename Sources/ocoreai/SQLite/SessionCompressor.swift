...
actor SessionCompressor {
    ...
    init(
        ...
        self.logger = Logger(label: "ocoreai.session")
        self.store = store
        self.fts = fts
        self.hotWindow = 50
        self.tokenThreshold = Self.DefaultTokenThreshold
        self.ttlDays = 180
    ...
    private func addUnsafe(
        ...
        do {
            if let calls = toolCalls {
                toolCallsJson = String(data: try JSONEncoder().encode(calls), encoding: .utf8)
            } else {
                toolCallsJson = nil
            }
        }
        ...
        do {
            ...
            if let json = toolCallsJson {
                params = [sessionId, role, content, now, tokenCount, json]
            } else {
                params = [sessionId, role, content, now, tokenCount, ""]
            }
            ...
            if sessionTokenCounts[sessionId, default: 0] >= tokenThreshold {
                self.logger.warning("Session \(sessionId) token count \(sessionTokenCounts[sessionId, default: 0]) exceeds threshold \(tokenThreshold)")
    ...
    private func triggerCompression(_ sessionId: Int64) {
        ...
        Task { [self] in
            await self.pruneColdMessages(sessionId, tokenCount: tokenCount)
        }
    }
    ...
    private func pruneColdMessages(_ sessionId: Int64, tokenCount: Int) async {
        ...
        do {
            ...
            self.logger.warning("Failed to fetch cold messages for session \(sessionId): \(error.localizedDescription)")
            return
        }
        ...
        let summary = await self.generateCompressionSummary(coldMessages)
        ...
        do {
            ...
            self.logger.warning("Failed to update session summary: \(error.localizedDescription)")
        }
        ...
        do {
            ...
            self.logger.info("Session \(sessionId) compressed — kept last \(hotWindow) of ~\(tokenCount) tokens")
            self.compressedSessions.insert(sessionId)
        } catch {
            self.logger.warning("Failed to compress session \(sessionId): \(error.localizedDescription)")
        }
    }
    ...
    private func generateCompressionSummary(_ messages: [(role: String, content: String)]) async -> String {
        ...
        if let llmCallback = llmSummarizer {
            do {
                ...
                self.logger.info("LLM summary generated (\\(summary.count) chars)")
                return summary
            } catch {
                self.logger.warning("LLM summarization failed (\\(error)), falling back to rules")
            }
        }
    ...
    func extractMemoryEvents(userMessage: String, assistantMessage: String, sessionId: Int64) async -> [MemoryEvent] {
        ...
        for event in events {
            do {
                ...
                self.logger.warning("Failed to store memory event: \(error)")
            }
        }
    ...
    private func expandByCues(primaryIDs: [Int64], contexts: [String], tags: [String], limit: Int) async throws -> [MemoryEvent] {
        ...
        if !contexts.isEmpty {
            ...
            WHERE context IN (\(ctxPlaceholders))
              ...
              LIMIT ?
            """
            let spreadParams: [AnyHashable] = contexts.map { $0 } + primaryIDs.map { Int64($0) } + [limit / 2]
    ...
    func recallPermanentMemory(
        ...
        let typeList = memoryTypes.map { $0.rawValue }.joined(separator: "', '")
        var sql = """
        SELECT * FROM memory_events
...
extension SessionModel {
    ...
    init?(from row: [String: SendableValue]) {
        ...
        self.updatedAt = Date(timeIntervalSince1970: Double(updatedAtTs) / 1_000_000)
        self.messageCount = Self.getInt(row, "message_count") ?? 0
