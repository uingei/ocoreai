...
actor RateLimitProvider {
    ...
    ///   - dict: In-out dict holding IP bucket entries
    ///   - rate: Token refill rate for new buckets
    ///   - capacity: Bucket capacity for new buckets
    /// - Returns: Existing or newly created ``TokenBucket`` actor reference
    @discardableResult
    private func getOrCreateIPBucket(
        ...
        let newBucket = TokenBucket(rate: rate, capacity: capacity)
        dict[key] = BucketEntry(bucket: newBucket, lastUsed: now)
        return newBucket
    }
    ...
    private func extractClientIP(from request: Request) -> String {
        ...
        if let xff = request.headers[HTTPField.Name("x-forwarded-for")!] {
        ...
        if let xri = request.headers[HTTPField.Name("x-real-ip")!] {
            if !xri.isEmpty {
                return String(xri)
            }
        }
    ...
    private func cleanupStaleBuckets() {
        let now = ContinuousClock.now
        let threshold: Double = Self.staleTimeoutSeconds

        // Evict stale model buckets
        var modelStaleKeys: [String] = []
        for (key, entry) in modelBucketEntries {
            let age = Double(entry.lastUsed.duration(to: now).components.seconds)
            if age > threshold {
                modelStaleKeys.append(key)
            }
        }
        for key in modelStaleKeys {
            modelBucketEntries.removeValue(forKey: key)
        }

        // Evict stale IP buckets
        var ipStaleKeys: [String] = []
        for (key, entry) in ipBuckets {
            let age = Double(entry.lastUsed.duration(to: now).components.seconds)
            if age > threshold {
                ipStaleKeys.append(key)
            }
        }
        for key in ipStaleKeys {
            ipBuckets.removeValue(forKey: key)
        }

        guard !modelStaleKeys.isEmpty || !ipStaleKeys.isEmpty else { return }
        logger.info(
            "Rate limit: evicted \\(modelStaleKeys.count) model + \\(ipStaleKeys.count) IP buckets, \\(ipBuckets.count) IP + \\(modelBucketEntries.count) model remaining",
            metadata: [
                "model_stale": .string(String(modelStaleKeys.count)),
                "ip_stale": .string(String(ipStaleKeys.count)),
            ]
        )
    }
...
struct RateLimitMiddleware<Context: RequestContext>: Sendable, RouterMiddleware {
    ...
    func handle(
        ...
        if let retryAfter = result.retryAfter {
            headers[HTTPField.Name("Retry-After")!] = String(format: "%.0f", retryAfter)
        }
        headers[HTTPField.Name("X-RateLimit-Limit")!] = "exceeded"
