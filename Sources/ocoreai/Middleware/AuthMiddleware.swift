...
struct AuthConfig: Sendable, Equatable {
    ...
    static let defaultPromptInjectionRegexes: [Regex<RegexOutput>] = [
        ...
        try! Regex(#"\bcontext:\s*you\s*are\b"#),
        try! Regex(#"\byou\s*are\s*(?:the\s*?(?:model|AI|assistant|GPT|ChatGPT|Claude))\b"#),
    ]
...
struct AuthMiddleware<Context: RequestContext>: Sendable, RouterMiddleware {
    ...
    private func extractAPIKey(from request: Request) throws -> String {
        ...
        if let bearer = request.headers[HTTPField.Name("authorization")!],
        ...
        if let apiKeyValue = request.headers[HTTPField.Name("api-key")!] {
