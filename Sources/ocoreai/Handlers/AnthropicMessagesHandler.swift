...
func anthropicMessagesHandler(
    ...
    // ═══════════════════════════════════════════════════════
    // Phase 1: Acquire inference session
    // ═══════════════════════════════════════════════════════
        ...
        do {
            ...
            do {
                for try await event in tokenStream {
                    ...
                    case .done(let reason):
                        let openaiReason = stopReasonToString(reason) ?? "stop"
                        stopReason = openAIToAnthropicStopReason(openaiReason)
            ...
            let errorEvent = AnthropicStreamEvent(
                type: "error",
                usage: AnthropicStreamUsage(inputTokens: 0, outputTokens: 0)
            )
