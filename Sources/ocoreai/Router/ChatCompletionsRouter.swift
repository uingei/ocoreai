...
func buildRouter(
    ...
    routes.post("/mcp") { request, context in
    ...
    routes.get("/sessions") { request, context in
    ...
    routes.delete("/sessions/:id") { request, context in
    ...
    routes.get("/sessions/:id/memory") { request, context in
        let idParam = try context.parameters.require("id")
        guard let id = Int64(idParam) else {
            throw AppError.invalidRequest("Invalid session ID: \(idParam)")
        }
    ...
    routes.get("/sessions/search") { request, context in
    ...
    routes.get("/skills") { _, context in
...
struct MemoryResponse: Sendable, Codable {
    let session_id: Int64
    let messages: [MessageModel]
}

struct SessionDeleteResponse: Sendable, Codable {
    let deleted: Bool
    let id: Int64
}
