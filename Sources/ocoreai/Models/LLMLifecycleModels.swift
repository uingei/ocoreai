...
struct TrainRequest: Sendable, Decodable {
    var model: String
    var trainingData: TrainingDatasetInput
    var validationData: TrainingDatasetInput?
    var lora: LoRAConfig = LoRAConfig()
    var hyperparams: TrainHyperparams = TrainHyperparams()

    enum TrainingDatasetInput: Sendable, Decodable {
        case inline([String])
        ...
        init(from decoder: Decoder) throws {
            ...
            if container.contains(.inline) {
                self = .inline(try container.decode([String].self, forKey: .inline))
                ...
                self = .inline(try [String].init(from: decoder))
            }
        }
    }

    struct LoRAConfig: Sendable, Decodable {
        var rank: Int = 8
        var scale: Float = 20.0
        var numLayers: Int = 4
    }

    struct TrainHyperparams: Sendable, Decodable {
...
struct EvalRequest: Sendable, Decodable {
    var model: String
    var dataset: [String]
    var batchSize: Int = 4
...
struct TrainSummary: Sendable, Encodable {
    var success: Bool
...
struct EvalSummary: Sendable, Encodable {
    var loss: Float
