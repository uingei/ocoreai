...
struct DownloadModelRequest: Sendable, Codable {
    let model: String
    let provider: String?
    let revision: String?
    let useLatest: Bool

    var effectiveProvider: String { provider ?? "hf" }
...
struct DownloadSSEEvent: Sendable, Codable {
...
struct DownloadStatusResponse: Sendable, Codable {
    let downloadId: String
