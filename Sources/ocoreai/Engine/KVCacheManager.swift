...
actor KVCacheManager {
    ...
    struct Config: Sendable {
        ...
        enum EvictionPolicy: String, Sendable {
    ...
    func gpuUsageGB() -> Double {
        return Double(gpuBytesUsed.load(ordering: .relaxed)) / (1024 * 1024 * 1024)
    }
...
final class AsyncKVState: Sendable {
    ...
    init(
        ...
        self.config = config
        self._keyCacheData = Self.arrayToData(keyCache)
        self._valueCacheData = Self.arrayToData(valueCache)
