...
actor MLXModelLoader {
    ...
    static func isModelCached(
        _ provider: MLXModelLoader.HubProvider, repoId: String, logger: Logger
    ) -> Bool {
    ...
    private static func hasValidSafetensors(in url: URL, log: Logger?) -> Bool {
        ...
        do {
            ...
            for item in files where item.pathExtension == "safetensors" {
                ...
                log?.warning(
                    "Incomplete cache: \(item.lastPathComponent) is 0 bytes — model will re-download"
                )
            }
            return false
        } catch {
        ...
        func loadFromHubWithFallback() async throws -> MLXLMCommon.ModelContainer {
            do {
                return try await loadFromHub(
                    provider, repoId: repoId, modelId: modelId, progressHandler: progressHandler)
            } catch {
                logger.warning(
                    "\(provider == .modelScope ? "ModelScope" : "HuggingFace") failed for \(modelId) — falling back: \(error.localizedDescription)"
                )
                let fallback: HubProvider = provider == .modelScope ? .huggingFace : .modelScope
                return try await loadFromHub(
                    fallback, repoId: repoId, modelId: modelId, progressHandler: progressHandler)
            }
        }
    ...
    func loadLocal(_ path: Path, modelId: String) async throws -> MLXLMCommon.ModelContainer {
        ...
        do {
            ...
            throw MLXLoadError.localLoadFailed(
                path: path.rawValue, error: error.localizedDescription)
        }
    }
    ...
    func loadLocal(url: URL, fallback modelId: String) async throws -> MLXLMCommon.ModelContainer {
        do {
            ...
            logger.warning(
                "Local path failed for \(modelId), trying hub: \(error.localizedDescription)")
        }
    ...
    @inline(never)
    func loadFromHub(
        _ provider: MLXModelLoader.HubProvider, repoId: String, modelId: String,
        progressHandler: (@Sendable (Progress) -> Void)? = nil
    ) async throws -> MLXLMCommon.ModelContainer {
        let start = ContinuousClock.now
        ...
        let progressKey =
            modelId.hasPrefix("mscope:")
            ? String(modelId.dropFirst(7))
            : modelId.hasPrefix("hf:")
                ? String(modelId.dropFirst(3))
                : modelId.hasPrefix("huggingface:")
