...
public struct KVBlock: Sendable {
    public static func == (lhs: KVBlock, rhs: KVBlock) -> Bool { lhs.blockId == rhs.blockId }
    public func hash(into h: inout Hasher) { h.combine(blockId) }
    ...
    init(blockId: Int, estimatedBytes: Int) {
        ...
        self.refCount = 0    // 0: 刚分配，尚未被任何 BlockTable 引用；调用方负责 addReference
        self.tokensUsed = 0
...
actor BlockPool {
    ...
    /// 池使用率（0.0-1.0）。
    public func usageFraction() -> Double {
    ...
    /// 估算内存占用的 GB 值。
    public func estimatedGB() -> Double {
    ...
    public func blockInfo(blockId: Int) -> KVBlock? {
        blocks[blockId]
    }

    /// 获取池统计信息。
    public func stats() -> BlockPoolStats {
        BlockPoolStats(
