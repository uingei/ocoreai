...
// `import simd` trimmed (unused in ocoreai build); `ImageStrategy` lives in
// CoreAIImageStrategy.swift.
...
/// normalizes with vectorized vDSP math.
struct ImagePreprocessor: Sendable {
    let targetSize: CGSize
    let mean: (CGFloat, CGFloat, CGFloat)
    ...
    static let gemma3 = ImagePreprocessor(
    ...
    /// CLIP preset (336x336).
    static let clip = ImagePreprocessor(
    ...
    func preprocess(imageURL: URL) throws -> (Data, Int, Int) {
    ...
    func preprocess(image: CIImage) throws -> (Data, Int, Int) {
    ...
    func preprocess(cgImage: CGImage) throws -> (Data, Int, Int) {
    ...
    func preprocessCHW(cgImage: CGImage) throws -> [Float] {
    ...
    func preprocessCHWCenterCrop(cgImage: CGImage) throws -> [Float] {
        ...
        guard
            let cropped = resized.cropping(
                to: CGRect(x: cropX, y: cropY, width: targetW, height: targetH))
        else {
            throw ImagePreprocessorError.renderFailed
        }
    ...
    func preprocessCHWPad(cgImage: CGImage) throws -> [Float] {
        let targetW = Int(targetSize.width)
        let targetH = Int(targetSize.height)
        let srcW = cgImage.width
        let srcH = cgImage.height
    ...
    func preprocessCHW(cgImage: CGImage, strategy: ImageStrategy) throws -> [Float] {
        switch strategy {
    ...
    private func normalize(pixels cgImage: CGImage, width w: Int, height h: Int) throws -> (
        Data, Int, Int
    ) {
        guard let colorSpace = CGColorSpace(name: CGColorSpace.sRGB) else {
            throw ImagePreprocessorError.renderFailed
...
enum ImagePreprocessorError: Error, LocalizedError {
    case loadFailed(URL)
    case renderFailed
