    ...
    static func create(
        ...
        do {
            ...
            let specializationOptions = SpecializationOptions(
                preferredComputeUnitKind: computeUnitKind)

            if let cache {
                _ = try await AIModel.specialize(
                    contentsOf: modelURL, options: specializationOptions, cache: cache)
            } else {
                _ = try await AIModel.specialize(
                    contentsOf: modelURL, options: specializationOptions)
            }
            let end = ContinuousClock.now
            logger.info(
                "Specialization completed in \(String(format: "%.1fms", Double((end - start).components.seconds) * 1000 + Double((end - start).components.attoseconds) / 1e15))"
            )
            ...
            logger.warning(
                "Core AI specialization failed (\(errorMsg)) — falling back to EngineFactory")
            return CoreAIModelHandle(
                isSpecialized: false,
                computeUnitKind: computeUnitKind,
        ...
        if enabled {
            ...
            if !fileManager.fileExists(atPath: Self.defaultCachePath) {
                try? fileManager.createDirectory(
                    atPath: Self.defaultCachePath, withIntermediateDirectories: true)
            }
        }
    }
