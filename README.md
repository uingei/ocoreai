...
- **Speculative decoding** — Gemma drafter model with per-model awareness (12B/26B/31B), MTP support with model-id isolation. Upstream pin `2af378b` includes KVCacheRound staged rounds (#516), TurboFlash short-context fast-path (#520), Qwen3MoE sanitization (#490), balanced chunking via `PrefillParameters` (#470).
