...
struct RequestModelTests {
    ...
    func testMinimalRequest() {
        let json = """
        {"model":"my-model","messages":[{"role":"user","content":"hello"}]}
        """
        let req = try #require(try JSONDecoder().decode(ChatCompletionRequest.self, from: json.data(using: .utf8)!))
