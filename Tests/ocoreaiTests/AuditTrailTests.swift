...
struct AuditTrailQueryTests {
    ...
    func queryByTool() async {
        ...
        let t1 = await trail.beginCall(
            caller: "agent", toolName: "tool_a", toolset: "test", arguments: [:])
        await trail.completeToken(t1, status: .success, result: "ok")

        let t2 = await trail.beginCall(
            caller: "agent", toolName: "tool_b", toolset: "test", arguments: [:])
        await trail.completeToken(t2, status: .success, result: "ok")

        let t3 = await trail.beginCall(
            caller: "agent", toolName: "tool_a", toolset: "test", arguments: [:])
    ...
    func queryByCaller() async {
        ...
        let t1 = await trail.beginCall(
            caller: "agent1", toolName: "tool", toolset: "test", arguments: [:])
        await trail.completeToken(t1, status: .success, result: "ok")

        let t2 = await trail.beginCall(
            caller: "agent2", toolName: "tool", toolset: "test", arguments: [:])
        await trail.completeToken(t2, status: .success, result: "ok")
    ...
    func recentRespectsLimit() async {
        ...
        for i in 0 ..< 20 {
            let token = await trail.beginCall(
                caller: "agent", toolName: "tool_\(i)", toolset: "test", arguments: [:])
    ...
    func tokenPreservesCaller() async {
        let trail = makeTrail()
        let token = await trail.beginCall(
            caller: "specific-agent", toolName: "t", toolset: "ts", arguments: [:])
        await trail.completeToken(token, status: .success, result: "ok")
...
struct AuditTrailEnforceTests {
    ...
    func clearRemovesAll() async {
        ...
        let token = await trail.beginCall(
            caller: "agent", toolName: "tool", toolset: "test", arguments: [:])
        await trail.completeToken(token, status: .success, result: "ok")
...
struct AuditTrailExportTests {
    ...
    func validJsonExport() async {
        ...
        let token = await trail.beginCall(
            caller: "agent", toolName: "export_tool", toolset: "test", arguments: ["key": "val"])
        await trail.completeToken(token, status: .success, result: "ok")
    ...
    func exportIncludesFields() async {
        ...
        let token = await trail.beginCall(
            caller: "agent", toolName: "full_fields", toolset: "test", arguments: ["x": "y"])
