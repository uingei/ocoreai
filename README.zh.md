...
- **双通道推理引擎** — MLX（Metal GPU，默认，`MLXLanguageModel` + `ChatSession` 管线双通道端侧推理）+ CoreAI（~3,090 LOC 跨 4 个引擎文件，派生自 Apple coreai-models 参考实现，动态 KV Cache、`TokenHistory` prefix caching）。零网络调用 — 推理在你的 Mac 上运行。
...
|- **推测解码** — Gemma drafter 模型支持（12B/26B/31B 独立路由），MTP 模式已接入。上游同步 2026-08-14 (mlx-swift-lm 5a81319): ReasoningEventEmitter ✅ (12 code refs)、KVCacheRuntime ✅ (turboQuant/affine)。缺口：upstream ThinkingBudget hard-budget enforcement ❌ (P1)、AgentLoop×SessionPool ❌ (P1)、CoreAI grammar ❌ (P1)。详见 `CHANGELOG.md` + 上游对齐报告。
